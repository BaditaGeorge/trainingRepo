...
// DotShape.prototype.dragDrop = undefined;
// DotShape.prototype.drop = undefined;
// DotShape.prototype.drag = undefined;
...
DotShape.prototype.moveAtDrag = function(posObj) {
    ...
    // }
    if (this.direction === 'x') {
        this.svgPth.setAttributeNS(null, 'x', posObj.x);
        this.startX = posObj.x;
    } else if (this.direction === 'y') {
        this.svgPth.setAttributeNS(null, 'y', posObj.y);
        this.startY = posObj.y;
    } else if (this.direction === 'd') {
        this.svgPth.setAttributeNS(null, 'x', posObj.x);
        this.svgPth.setAttributeNS(null, 'y', posObj.y);
    ...
    for (let i = 0; i < this.dots.length; i++) {
        ...
        if (this.dots[i].length > 1) {
            if (this.dots[i][1].svgPth !== undefined) {
                this.dots[i][1].svgPth.setAttributeNS(null, 'y', -20);
            }
        }
    }
...
DotManager.prototype.moveElements = function(element) {
    if (this.dots[0].length > 1) {
        for (let i = 0; i < this.dots.length; i++) {
            this.dots[i][1].svgPth.setAttributeNS(null, 'x', this.computePosition(this.dots[i][0].position, element)[0]);
            this.dots[i][1].svgPth.setAttributeNS(null, 'y', this.computePosition(this.dots[i][0].position, element)[1]);
        }
    }
}
